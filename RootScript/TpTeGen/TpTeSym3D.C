...
// The electron/positron is generated with uniform random Energy and Solid angle in 3D.
...
void TpTeSym3D (){
  ...
  */
  ...
  //Histogram
  TH1F * h_thetap = new TH1F("h_thetap", "cos(Thetap)",
				300, 0, 1);
  h_thetap->SetXTitle("cos(Thetap)");
  h_thetap->SetYTitle("Counts");
  
  TH1F * h_thetae = new TH1F("h_thetae", "cos(Thetae)",
				300, 0, 1);
  h_thetae->SetXTitle("cos(Thetae)");
  ...
  for (Int_t i = 0; i < N; i++)
	...
	}while (Ee < Me || ((E-Ee)*(E-Ee)-Pey*Pey-Pez*Pez-Me*Me) < 0.0 || Thetae>(PI/8));
	...
	Thetae = Thetae*180/PI;
	  ...
	  Phip = gRandom->Uniform(0, 2*PI);
	  Thetap = gRandom->Uniform();
	  Thetap = acos(-2*Thetap+1);
	...
	}while (Ep < Me || ((E-Ep)*(E-Ep)-Ppy*Ppy-Ppz*Ppz-Me*Me) < 0.0 || Thetap>(PI/8));
    
	Thetae = atan2(sqrt(Pey*Pey+Pez*Pez),Pex);
	Thetae = Thetae*180/PI;
	Thetap = Thetap*180/PI;
      }
	...
	Thetap = Thetap*180/PI;
      ...
      h_thetap->Fill(cos(Thetap*PI/180));
      h_thetae->Fill(cos(Thetae*PI/180));
      */
