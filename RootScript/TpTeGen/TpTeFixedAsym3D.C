...
void TpTeFixedASym3D (){
  ...
  Double_t del[Nm], N0[8][Nm], A0[8][Nm], AConst[Nm], ThetaP = 161.0;
  ...
  for (Int_t i = 0; i < N; i++)
	...
	do{
	  ...
	  //do{
	    ...
	    Thetap = atan2((sqrt(Ee*Ee-Me*Me)*sin(Thetae)),Ppx);
	    Phip = Phie + PI;
	    ...
	    if (Thetae > 0){
	      ...
	      Thetap = atan2(sqrt(Ee*Ee-Me*Me)*sin(abs(Thetae*1.0)),Ppx);}
	    */
